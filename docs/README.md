...
| **ExitBlueprint Bench** (groundwork: two-axis deliverable eval — answer + source score; deterministic checks reuse `numeralPostCheck`; LLM-judge stubbed) | `server/llm/evals/bench.ts` · `server/llm/evals/ci.ts` · `server/llm/evals/rubrics/*` | sellside-ai/02 |
