...
// Synthesis model. WS-COPILOT specifies the 'standard' tier for synthesis (economy
// is acceptable for tool selection). This branch has no server/llm/models.ts
// modelForTier() helper, so the standard-tier id is named directly here; the same
// loop drives tool selection, so there is one model. resolveProvider().modelFor
// namespaces it to the gateway slug at call time.
